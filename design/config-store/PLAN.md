...
## 6. Settings page IA + wizard screens

**Design of record: the v2 designer prototype at `design/config-store/prototype/NIM Proxy Dashboard.dc.html`** (directional guide, not pixel-for-pixel — maintainer's instruction). It confirms this section and refines it; where the prototype and this text differ, the prototype wins on look, this plan wins on behavior/data. Verified against it (rendered + screenshotted, all roles/sections/wizard steps):
- Settings is a **sub-nav layout** (left rail inside the page: Access & keys · Server · Users · Account; Server/Users hidden for user role), not stacked cards. A role-preview pill row exists in the prototype only — the real app derives role from the session.
- **Key rows** carry: masked key + owner chip, `fp <8-hex>` fingerprint, lane index (`unassigned` when disabled), validate state (`✓ 12 models`), live lane state (`31 / 40 in window`, `cooldown 4s`), inline rpm input, enable toggle, delete — which becomes a **padlock on the superuser's guarded key** (the invariant, visualized). Card header shows "`N` enabled in pool" (aggregate visible to all roles; rows filtered by ownership).
- **Client API keys**: generated secrets use an **`npk_` prefix** (adopt as the generation format), masked to last-4, owner chip (admin view), red Revoke.
- **Server section**: API-access-mode card (Open/Keyed) on top; Upstream & limits with unit-suffixed inputs; the **model-pressure governor card** (adaptive master toggle + per-model override chips like `Nemotron 3 Super 120B [16 max] ×` + "+ override"); Pricing & history combined.
- **Users section**: monogram + username, role badge (SUPERUSER/ADMIN/USER), key counts ("2 NIM · 2 client"), Reset password / Role / Delete (no Delete on superuser); add form = username + initial password + user/admin toggle. Header copy: "deleting a user pulls their NIM keys from the pool & stops their harnesses".
- **Connection card** (all roles): client base URL (`http://host:port/v1`) with copy button + current API mode. Display nuance: the *client-facing* URL shown includes `/v1`; the stored upstream `base_url` does not — keep the two labeled distinctly.
- **Wizard**: numbered-step indicator (1 Create account → 2 Add NIM keys → 3 Review & finish), validated-key chips (`✓ validated · 12 models · 40 rpm`), "Advanced — upstream base URL" disclosure, Back/Continue.
- **No new observability charts in the v2 bundle** (chart-for-chart identical to v1 on all five tabs — verified by markup diff). Governor observability is therefore specified here, not by the prototype (next section).

### Governor observability (added — the new metrics deserve a surface)
A **Model pressure** card on the Reliability tab, rendered only once the governor has ever engaged in the window (hidden otherwise — zero noise for unaffected deployments): worker-exhaustion events/min (lineChart) + per-model `inflight vs limit` bar rows (barList, `limit` labeled; ungoverned models absent). Built entirely from the existing primitives + the new `nimproxy_worker_exhausted_total{model}` / `nimproxy_model_inflight` / `nimproxy_model_limit` series. Lands with PR 5 (Settings UI).

### Original IA notes (superseded on layout by the prototype, kept for field/behavior detail)
...
5. **Settings UI + wizard polish + Model-pressure card + docs/knowledge**: dashboard Settings tab per §6 (sub-nav layout, role-conditional sections, `npk_` secret format, padlocked guarded key) + the Reliability Model-pressure card; new ADR `ui-managed-config-store.md` (covers multi-user model + ownership + no-encryption rationale + claim-risk acceptance); amend `auth-posture` ADR; rewrite `ops/configure-env.md` (5 container vars); update `.env.example`, README (quickstart → first-run wizard; sharing-with-friends flow now "create them a user"), `deploy-docker.md` (volume backup now includes config.json), `client-auth.md`, `ops/sharing-with-friends.md`, `examples/README.md`, CHANGELOG, log.md. *Verify*: Playwright walkthrough as superuser/admin/user (each sees the right sections; raw `/api/config` bodies diff clean per role) + restart round-trip; screenshot comparison against the v2 prototype (directional); binary-size check vs ~3.5MB budget (expect +100–200KB, measured and recorded).
